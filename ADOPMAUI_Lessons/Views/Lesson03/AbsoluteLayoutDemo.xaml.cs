...
namespace ADOPMAUI_Lessons.Views.Lesson03
{
	public partial class AbsoluteLayoutDemo : ContentPage
	{
		...
		async void UpdateSize()
		{
			...
			AbsoluteLayout.SetLayoutBounds(box, new Rect(0f, 0f, w, h));
			AbsoluteLayout.SetLayoutBounds(anchorVert, new Rect(.5, 0f, AbsoluteLayout.AutoSize, AbsoluteLayout.AutoSize));
		...
		async void UpdatePosition()
		{
			...
			float x = 0.0f;
			float y = 0.0f;
			AbsoluteLayout.SetLayoutBounds(box, new Rect(x, y, .25, .25));
			AbsoluteLayout.SetLayoutBounds(anchorVert, new Rect(x, y + (y * box.Height), 5, 5));
